    ...
    >
      ...
      <div class="alert-text">
        <router-link to="/simpanan/tambah">
          <b-button class="mr-3" variant="success">Tambahkan Simpanan</b-button>
        </router-link>
...
export default {
  ...
  computed: {
    ...mapState({
      header: (state) => state.rekening.simpanan.header,
      items: (state) => state.rekening.datasimpanan,
    }),
    search: {
      get() {
        return this.$store.state.rekening.simpanan.search;
      },
      set(val) {
        this.$store.commit("master/setSimpananSearch", val);
      },
    },
  },
};
